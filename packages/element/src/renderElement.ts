...
import { renderShaderElementSync } from "./skslEngine";
...
const drawElementOnCanvas = (
  ...
  // Intercept SkSL Shader elements
  if (element.customData && element.customData.shader) {
    const shaderType = element.customData.shader as 'neon' | 'gradient_mesh';
    const shaderCanvas = renderShaderElementSync(element.width, element.height, shaderType, (skCanvas: any, paint: any) => {
      skCanvas.drawRect([0, 0, element.width, element.height], paint);
    });

...
class Collab extends PureComponent<CollabProps, CollabState> {
  ...
  stopCollaboration = (keepRemoteState = true) => {
    ...
    if (this.portal.socket && this.fallbackInitializationHandler) {
      this.portal.socket.onerror = null;
    }
  ...
  startCollaboration = async (
    ...
    const wsUrl = import.meta.env.VITE_APP_WS_SERVER_URL.replace(/^http/, 'ws');
    const userId = this.state.username || crypto.randomUUID();
    ...
    try {
      ...
      this.portal.socket.onerror = fallbackInitializationHandler;
    } catch (error: any) {
      console.error(error);
    ...
    this.portal.socket.onmessage = async (event: MessageEvent) => {
      ...
      try {
        ...
        // Emulate new-user
        if (envelope.event_type === "SYSTEM_JOIN_ROOM" && envelope.sender_id !== userId) {
        ...
        const decryptedData = envelope.payload;
    ...
    this.portal.socket.addEventListener("open", async () => {
    ...
    // Removed USER_FOLLOW_ROOM_CHANGE as socket.io handles it natively
  ...
  setCollaborators(sockets: SocketId[]) {
    ...
    for (const socketId of sockets) {
      const isCurrentUser = socketId === this.portal.userId;
      collaborators.set(
        socketId,
        Object.assign(
  ...
  setUsername = (username: string) => {
    ...
    const socketId = this.portal.userId as SocketId | undefined;
